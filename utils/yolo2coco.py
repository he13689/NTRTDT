...
parser.add_argument('--root_dir', default='data_new_yolo/', type=str, help="root path of images and labels, include ./images and ./labels and classes.txt")
parser.add_argument('--save_path', type=str, default='./train.json', help="if not split the dataset, give a path to a json file")
parser.add_argument('--random_split', default=True, help="random split the dataset, default ratio is 9:0.1:0.9")
...
def train_test_val_split_random(img_paths, ratio_train=0.9, ratio_test=0.02, ratio_val=0.98):
    # 这里可以修改数据集划分的比例。
...
def yolo2coco(arg):
    ...
    indexes2 = os.listdir(originImagesDir)
    ...
    if arg.random_split or arg.split_by_file:
        ...
        if arg.random_split:
            ...
            train_img, val_img, test_img = train_test_val_split_random(indexes2, 0.9, 0.098, 0.002)
    ...
    # 标注的id
    ann_id_cnt = 0
    for k, index in enumerate(tqdm(indexes2)):
        # 支持 png jpg 格式的图片。
        if os.path.isdir(root_path + '/'+index):
            continue
        txtFile = index.replace('.jpg', '.txt').replace('.png', '.txt').replace('.JPG', '.txt')
        # 读取图像的宽和高
